...
### Changed
- **CI/CD Build Stability**: Added `submodules: recursive` checkout step for `vendor/rar` dependency resolution on GitHub Actions runners, and fixed Python `scripts/generate_licenses.py` file `FileNotFoundError` caused by using Unix `/tmp` pathways on native Windows runners.
