...
### Changed
- **Documentation Truthfulness Audit**: Corrected heavily misleading technical claims in `docs/index.html`. Refined the "Executable Bit Stripping" claim to accurately reflect its `(Unix Only)` structural constraint, and correctly redefined the "WASI Constraints" to depict the accurate single `preopened_dir` layer instead of fabricated read/write stream handles.
